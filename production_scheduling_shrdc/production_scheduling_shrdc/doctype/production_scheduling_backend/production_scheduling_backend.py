...
def production_scheduling(doc):
	...
	wo_qty,last_datetime = find_datetime(pp_date,start_time,end_time)
	...
	for row in po_items:
		...
		last_datetime,temp,exceed = check_within_operating_hours(d,last_datetime,pp_date,start_time,end_time,total_op_time)
		if (~exceed):
			d.planned_start_date = last_datetime # Set the child doctype planned start date as the last_datetime
		last_datetime = temp #Assign the new datetime to the last_datetime
...
def find_datetime(pp_date,start_time,end_time):
...
def check_within_operating_hours(cd,last_datetime,pp_date,start_time,end_time,total_op_time):
	delay = 5
	exceed = 0
	...
	temp = add_to_date(last_datetime,minutes=(predict_op_time),as_datetime=True) #Predict the end datetime

	if start_time and end_time:
		# datetime.datetime(2021, 12, 28, 12, 00, 00)
		if (temp <= end_time): # check whether the operation time over the working hour
			...
			#cd.planned_qty = time_left/total_op_time
			cd.planned_qty = round(round(time_left)/total_op_time)
			...
			wo_qty,last_datetime = find_datetime(pp_date,start_time,end_time) #Get the word order list of next day
			last_datetime,temp,exceed = check_within_operating_hours(cd,last_datetime,pp_date,start_time,end_time,total_op_time)
			# Reassign back the last_datetime so it match what we want

	return last_datetime,temp,exceed
